...
namespace dice {
	...
	public:
		uint16_t roll(uint16_t nbr_side) {
			...
			uint16_t res = getRandomValue(nbr_side);
			std::cout << "Roll: " << res << '\n';
		...
		uint16_t rollAdvantage(uint16_t nbr_side) {
			...
			uint16_t roll_1 = getRandomValue(nbr_side);
			uint16_t roll_2 = getRandomValue(nbr_side);
		...
		const std::shared_ptr<IObject>& roll(const std::array<std::shared_ptr<IObject>, N>& obj_array) {
			...
			std::shared_ptr<IObject>& obj = obj_array[getRandomValue(obj_array.size()) - 1];
			obj->printObject();
	...
	private:
		uint16_t getRandomValue(const uint16_t max_value) {
